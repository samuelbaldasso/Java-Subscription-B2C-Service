...
import com.sbaldasso.b2c_subscription_service.domain.exception.OptimisticLockException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleOptimisticLockException(Exception ex) {
        log.error("Optimistic lock exception - concurrent update detected", ex);

        ErrorResponse response = ErrorResponse.builder()
                .timestamp(LocalDateTime.now())
                .status(HttpStatus.CONFLICT.value())
                .error("Concurrent Update Detected")
                .message("The resource was updated by another process. Please retry your operation.")
                .build();
